...
Socket::Socket(const char *hostname, const char *servicename) : skt(-1), closed(true) {
    ...
    if (skt != -1)
        ::close(skt);
    throw std::runtime_error("Error al instanciar el socket al host y al servicio");
...
Socket::Socket(const char *servicename) : skt(-1), closed(true) {
    ...
    if (skt != -1)
        ::close(skt);
    throw std::runtime_error("Error al instanciar el socket al servicio");
    // TODO lanzar excepcion
}
    ...
    *was_closed = false;
    ...
    while (received < sz) {
        ...
        if (s < 0) {
            ...
            throw std::runtime_error("Error al intentar recibir bytes.");
        } else if (s == 0) {
    ...
    *was_closed = false;
    ...
    while (sent < sz) {
        ...
        if (s < 0) {
            ...
            // deberiamos entonces checkear los errores y no solo retornarlos)
            throw std::runtime_error("Error al intentar enviar bytes.");
        } else if (s == 0){
            // El socket fue cerrado
            throw s;
        } else {
