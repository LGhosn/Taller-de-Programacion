...
Socket::Socket(const char *hostname, const char *servicename) : skt(-1), closed(true) {
    ...
    if (skt != -1)
        ::close(skt);
    throw SocketExceptions("Error al instanciar el socket al host '%s' y al servicio '%s'",
...
Socket::Socket(const char *servicename) : skt(-1), closed(true) {
    ...
    if (skt != -1)
        ::close(skt);
    throw SocketExceptions("Error al instanciar el socket al servicio '%s'", servicename);
    // TODO lanzar excepcion
}
    ...
    *was_closed = false;
    ...
    if (s == 0) {
        ...
        *was_closed = true;
        ...
        throw SocketExceptions("Error al recibir datos");
    } else {
        return s;
    }
}
    ...
    *was_closed = false;
    ...
    if (s == 0) {
        ...
        *was_closed = true;
        ...
        throw SocketExceptions("Error al enviar datos");
    } else {
        return s;
    }
}
    ...
    *was_closed = false;
    ...
    while (received < sz) {
        ...
        if (s < 0) {
            ...
            throw SocketExceptions("Error al intentar recibir %lu bytes. Recibidos: %i bytes",
                 sz, received);
        } else if (s == 0) {
            // El socket fue cerrado
            throw SocketExceptions("Error al intentar recibir %lu bytes. El socket fue cerrado",
                 sz);
        } else {
...
Socket Socket::accept() {
    ...
    if (skt == -1) {
       throw SocketExceptions("Error al aceptar conexion");
    }
    //    TODO lanzar excepcion
